...
import { allAsyncAPIFormatsPerVersion } from '../formats';
...
export function getSchema(docFormats: Set<Format>): Record<string, any> | void {
  Object.entries(allAsyncAPIFormatsPerVersion).forEach(([version, asyncAPIFormat]) => {
    if (docFormats.has(asyncAPIFormat)) {
